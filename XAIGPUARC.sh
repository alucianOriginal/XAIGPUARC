...
#|IMPERATOR|INTERPRATOR|EDITION|AUTOMATOR|
...
log "🔷ORNDER|FLASH|ATTENTION|'${CUSTOM_KERNEL_DIR}'ANGELEGT"
fi
...
log "🔷PATCH 4b|8 ERFOLGREICH|FLASH|ATTENTION|GELADEN"
else
...
#7|8
log "🔷PATCH 7|8: ERZWINGT|MAX|BLOCK|SIZE|1024|AUF|BIG|ARC"
if [ -f "$GGML_SYCL_CPP" ]; then
if ! grep -q "GGML_SYCL_MAX_BLOCK_SIZE 1024" "$GGML_SYCL_CPP"; then
sed -i '#define GGML_SYCL_MAX_BLOCK_SIZE 1024' "$GGML_SYCL_CPP"
success "✅PATCH 7|8 ERZWINGE|MAX|BLOCK|SIZE|1024|BIG|ARC|AKTIVIERT"
...
#8|8
log "🔷PATCH 8|8 OPTIMIERE|sycl::queue|ORDNER|XAIGPUARC"
...
error "❌PATCH 8|8 AENDERUNGEN|NICHT|FERTIG"
return 1
fi
else
warn "⚠️PATCH 8|8 sycl::queue|KONSTRUKTOR|NICHT|GEFUNDEN|UEBERSPRINGE"
fi
success "✅ALLE 7|8 EINGLIEDERUNGEN|INTEL|ARC|XE|iGPU|dGPU|eGPU|BASIERTE|XAIGPUARC|ERFOLGREICH|ANGEWANDT"
}
...
KONNTE|KEIN|GERAET|FINDEN|VERSUCHE|ZWEITEN|FALLBACKVERSUCH|BITTE|WARTEN"
fi
}
#7MODELLPFADWAEHLEN
prepare_model() {
...
main "${1:-1}" "${2:-}" "${3:-}"
#42
log "🔷KOMPLETTBAUVORGANG|GESPEICHERT|**${LOG_FILE}**"
