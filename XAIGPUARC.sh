...
warn "⚠️KEINE|INTERNETVERBINDUNG|GEFUNDEN|ERSTINSTALLATION|LADEN|ABHAENGIGKEITEN|NOTWENDIG|BITTE|ANSCHLUSS|PRUEFEN"
return 1
fi
}
#UMGEBUNGRUECKFALLMECHANISMENVORBEREITEN
prepare_environment() {
log "🔷HOLE|ONEAPI|KOPFZEILEN|FUER|BAU|VON|XAIGPUARC|"
local SETVARS_PATH="/opt/intel/oneapi/setvars.sh"
if [ ! -f "$SETVARS_PATH" ]; then
error "❌ONEAPI|KOEPFE|NICHT|GEFUNDEN|$SETVARS_PATH|INSTALLIERE|ERST|ONE|API|BIBLIOTHEKEN"
exit 1
fi
...
#1PROJEKTVORBAU
setup_project() {
log "🔷BAUE|VORBAU|XAIGPUARC|BITTE|BEIM|ERSTEM|BAU|WARTEN"
if [ ! -d "${LLAMA_CPP_DIR}" ]; then
log "🔷KLONE|GRUNDLAGEN|LLAMACPP"
git clone https://github.com/ggerganov/llama.cpp "${LLAMA_CPP_DIR}"
if [ $? -ne 0 ]; then
error "❌KLONEN|LAMACPP|FEHLGESCHLAGEN|ABBRUCH"
exit 1
fi
fi
...
log "🔷PATCH 1|8 MKL|MATHEMATIKBLIOTHEKENKOPFE|ERFOLGREICH|EINGELADEN|SPEICHERE|AUSGABE"
else
error "❌PATCH 1|8 MKL|MATHEMATIKBIBLIOTHEKEN|EINLADEN|FEHLGESCHLAGEN"
return 1
fi
else
...
log "🔷PATCH 3|8 ERFOLGREICH|ICP|X|KOPFZEILEN|EINGEFUEGT"
else
...
log "✅PATCH 4a|8 AWK|UNTERBAU|VORHANDEN|FLASH|ATTETNTION|INJEKTION|FORTFAHREN"
fi
log "🔷PATCH 4b|8 ERFOLGREICH|FLASH|ATTENTION|GELADEN"
else
error "❌PATCH 4b|8 FEHLGESCHLAGEN|FLASH|ATTENTION|AWK|KERN|NICHT|IN|HOME|GEFUNDEN"
return 1
fi
#5|8
log "🔷PATCH 5/8 INJIZIERE|FLASH|ATTENTION|OBJEKT|VARIABLEN|UNTERBLOCK|SYCL|BIBLIOTHEKEN"
local CMAKE_LISTS_FILE="${LLAMA_CPP_DIR}/ggml/src/ggml-sycl/CMakeLists.txt"
#5a|8
...
log "🔷PATCH 5b|8 ERFOLGREICHE|GGML|SYCL|INJEKTIONEN|BAUVORGANG"
else
...
success "✅ALLE 8|8 EINGLIEDERUNGEN|INTEL|ARC|XE|iGPU|dGPU|eGPU|BASIERTE|XAIGPUARC|ERFOLGREICH|ANGEWANDT"
}
#3XAIGPUARCBAUKONFIGURATION
configure_build() {
log "🔷BEREITE|XAIGPUARC|KOPFZEILENBAUVORGANG"
local FP_MODE="${1:-1}"
local SYCL_FLAGS="-DGGML_SYCL_F16=${FP_MODE} -fsycl-device-code-split=per_kernel"
if [ ! -d "${BUILD_DIR}" ]; then
log "🔷LEGE|XAIGPUARC|ORDNER|HOME|VERZEICHNIS|OMPUTERS|${BUILD_DIR}"
...
|INSTALLATION
|JE|NACH|LEISTUNG|SYSTEMS|
|MINUTEN|ANDAUERN|

|BITTE|GEDULD|
|DANKE|FUER|NUTZUNG|XAIGPUARC|

|||| ||||||||||| |
|ZWEITER|VORGANG|WESENTLICH|SCHNELLER|
| ||||||| ||||||||
