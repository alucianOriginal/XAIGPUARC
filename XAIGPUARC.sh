...
#|IMPERATOR|INTERPRATOR|EDITION|AUTOMATOR|GEHIRN|O|MAT|

#SYCL
#31|12|2025|18:25|
#Goldenbug|Imperator|Interprator|X|ai|ki|GPUARC|AUTOMATOR|GEHIRNOMAT|EIWEISSCOMPUTER|
...
#INTERNETPRUEFUNG
check_internet() {
log "🔷PRUEFE|INTERNET|VERBINDUNG"
if timeout 5 bash -c "</dev/tcp/8.8.8.8/53" 2>/dev/null; then
success "✅INTERNETVERBINDUNG|VORHANDEN"
return 0
else
warn "⚠️KEINE|INTERNET|VERBINDUNG|GEFUNDEN|ERSTINSTALLATION|LADEN|ABHAENGIGKEITEN|NOTWENDIG|ANSCHLUSS|PRUEFEN"
return 1
fi
}
...
error "❌ICX|IPCX||ONEAPI|INTEL|XAIGPUARC|BAUUNTERMODUL|INSTALLATION|FEHLGESCHLAGEN"
exit 1
fi
...
if [ $? -ne 0 ]; then
error "❌KLONEN|BAUSTEINE|FEHLGESCHLAGEN|ABBRUCH"
exit 1
fi
fi
...
error "❌FEHLER|HAUPT|VERZEICHNIS'${LLAMA_CPP_DIR}'NICHT|GEFUNDEN|ABBRUCH"
exit 1
fi
}
#PATCH|LOGIK|1|8|a|b|c
patch_llama_cpp() {
log "🔷PATCH|1-8|GGML|SYCL|ANLEGEN|KOPF|ZEILEN|MATHEMATIK|BIBLIOTHEK|REGESTRIERUNG"
...
log "🔷PATCH 1|8 MATHEMATIK|BIBLIOTHEK|KOPF|ZEILEN|LADEN"
if sed -i 's|#include <sycl/ext/oneapi/math.hpp>|#include <sycl/ext/intel/math.hpp>|g' "$DPCT_HELPER_FILE"; then
log "🔷PATCH 1|8 MATHEMATIK|BIBLIOTHEK|KOEPFE|LADEN|ERFOLGREICH|SCHREIBE|KOPFZEILEN"
...
log "🔷PATCH 1|8 MKL|MATHEMATIK|BIBLIOTHEKEN|KOEPFE|ERFOLGREICH|GELADEN|SPEICHERE|AUSGABE"
else
error "❌PATCH 1|8 MKL|MATHEMATIK|BIBLIOTHEKEN|EINLADEN|FEHLGESCHLAGEN"
return 1
fi
else
error "❌PATCH 1|8 MATHEMATIK|BIBLIOTHEKEN|MKL|FEHLGESCHLAGEN|NICHT|GEFUNDEN|ABHAENIGKEITEN|PRUEFEN"
return 1
fi
#2|8
...
log "🔷PATCH 2|8 ggml_flash_attention_sycl.cpp|KERNE|'.|${KERNEL_SOURCE_LOCAL}'|'${CUSTOM_KERNEL_SRC}'|KOPIERT"
fi
...
" > "$CUSTOM_KERNEL_CMAKE"
log "🔷PATCH 2a|8 CMAKE|TXT|LISTEN|OBJEKTE|KOPFZEILE|EINGEFUEGT"
#2b|8
...
log "🔷PATCH 2b|8 ERFOLGREICH|FLASH|ATTENTION|KOPFZEILEN|CMAKE|TXT|GESCHRIEBEN"
else
error "❌PATCH 2b|8 FLASH|ATTENTION|EINGLIEDERUNG|KOPFZEILEN|CMAKE|TXT|FEHLGESCHLAGEN"
return 1
fi
else
...
log "🔷PATCH 3|8 ERFOLGREICH|ICP|X|CMAKET|TXT|KOPFZEILEN|EINGEFUEGT"
else
...
error "❌PATCH 3a|8 FEHLGESCHLAGEN|CMAKE|LISTS|TXT|SYCL|GGML|PFADE|NICHT|GEFUNDEN|BITTE|ABHAENGIGKEITEN|AUF|GARUDA|Linux|ARCH|SYSTEM|PRUEFEN"
return 1
fi
#4|8
log "🔷PATCH 4|8 FLASH|ATTENTION|HAUPTKERN|KOPFZEILEN|INJIZIEREN"
...
if [ $? -eq 0 ]; then
log "🔷PATCH 4a|8 AWK|KOPFZEILEN|DEKLARATION|ERFOLGREICH|EINGEFUEGT"
else
error "❌PATCH 4a|8 FEHLER|EINFUEGEN|FLASH|ATTENTION|DEKLARATION|AWK|KOPFZEILEN"
return 1
fi
else
...
log "🔷PATCH 4a|8 FUEGE|ZWISCHENSPEICHER|PER|AWK|KOPF|ZEILE|EIN"
...
mv /tmp/ggml-sycl.cpp.new "${GGML_SYCL_CPP}"
if [ $? -eq 0 ]; then
log "🔷PATCH 4a|8 AWK|UNTERBAU|KOPFZEILEN|EINGEFUEHRT"
else
error "❌PATCH 4a|8 FEHL|EINFUEGEN|AWK|KOPFZEILEN"
fi
else
log "✅PATCH 4a|8 AWK|UNTERBAU|VORHANDEN|FLASH|ATTENTION|INJEKTION|FORTFAHREN"
fi
log "🔷PATCH 4b|8 ERFOLGREICH|FLASH|ATTENTION|AKW|UNTERBAU|GELADEN"
else
error "❌PATCH 4b|8 FEHLGESCHLAGEN|FLASH|ATTENTION|AWK|PATCH|KERN|NICHT|IN|HOME|GEFUNDEN"
return 1
fi
#5|8
...
#8|8
log "🔷PATCH 8|8 sycl::queue|ORDNER|HOME|XAIGPUARC|OPTIMIERVORGANG"
...
error "❌PATCH 8|8 sycl::queue|AENDERUNGEN|NICHT|FERTIG"
return 1
fi
else
warn "⚠️PATCH 8|8 sycl::queue|NICHT|GEFUNDEN|UEBERSPRINGE"
fi
...
configure_build() {
log "🔷BEREITE|XAIGPUARC|KOPFZEILEN|BAUVORGANG"
local FP_MODE="${1:-1}"
...
error "❌CMAKE|TXT|KOPFZEILEN|BAU|FEHLGESCHLAGEN"
return 1
fi
...
error "❌BAU|XAIGPUARC|KOPFZEILEN|FEHLGESCHLAGEN|UEBERPRUEFEN|**${BUILD_DIR}/${LOG_FILE}**"
return 1
fi
success "✅BAU|XAIGPUARC|ERFOLGREICH"
else
error "❌KONNTE|XAIGPUARC|NICHT|NEUBAUEN|'${BUILD_DIR}'|WEGEN|FEHLERHAFTEM|WECHSEL|BAU|NICHT|MOEGLICH"
return 1
fi
}
...
if [ -z "$DEVICES" ]; then
warn "⚠️KEINE|KOMPATIBLEN|SYCL|GERAETE|GEFUNDEN|SUCHE|ERNEUT|UEBERGEHEN|iGPU|NUTZUNG|VOR|dGPU|NUTZUNG|DUALGPU|WEITER|MIT|SINGLEGPU"
...
log "🔷AUTO|NGL|BERECHNUNG|**${N_GPU_LAYERS}**SCHICHTEN||JE|NACH|MODELL|PRIO|iGPU|dGPU|CPU|AUTO|VERTEILT"
fi
}
...
warn "⚠️KEIN|SYCL|GERAET|GEFUNDEN|${FULL_LS_PATH}.
KONNTE|KEIN|SYCL|FEHIGES|GERAET|FINDEN|VERSUCHE|ZWEITEN|RUECKFALL"
fi
}
#7MODELLPFADWAEHLEN
prepare_model() {
...
local CONTEXT_SIZE=16384 #NEUE WERTE SETZEN 512 1024 2048 4096 8192 16384 32768
...
ZES_ENABLE_SYSMAN=1 "${FULL_LLAMA_CLI_PATH}" \
    ...
    --split-mode none \
    --main-gpu ${GPU_ID}
echo "✅KI|ANTWORT|VORBEIREITUNG|FERTIG|GLUECKWUNSCH|KI|CHAT|FUNKTION|JETZT|AKTIV"
}
...
success "✅|XAIGPUARC|VORHANDEN|NEUBAU|UNNOETIG|**${FULL_LLAMA_CLI_PATH}**|**${FULL_LS_PATH}**|WEITER"
log "🔷UEBER|SPRINGE|BAU|VORGANG"
RERUN_BUILD=0
else
...
run_inference "${2:-}" "${3:-}"
#8
log "🔷|XAIGPUARC|ANTWORT|ABGESCHLOSSEN|CHAT|BEGINNT|STELLEN|SIE|IHRE|FRAGE|UND|DRUECKEN|SIE|ENTER|**${BUILD_DIR}/${LLAMA_CLI_PATH}**"
}
#HAUPTSCHLEIFE
main "${1:-1}" "${2:-}" "${3:-}"
#42
log "🔷KOMPLETT|BAUVORGANG|GESPEICHERT|**${LOG_FILE}**"
